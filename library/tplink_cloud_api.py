...
class TPLink:
    ...
    def set_relay_child_state(self, alias: str, child: str, state: int) -> Dict:
        """
        Turn off the device with the given alias.
        ...
        if not device:
            raise RuntimeError('Cannot find device: ' + alias)
        print('aliasaliasalias',alias)
        print('childchildchild', child)
        request = {
            'method': 'POST',
            'url': self.url,
            'params': {
            ...
            'data': {
                ...
                'params': {
                    ...
                    'requestData': {
                        'context': {
                            'child_ids': [ (device['deviceId'] + child) ]
                        },
                        'system': {
                            'set_relay_state': {
                                'state': state
    ...
    def turn_on_child(self, alias: str, child: str) -> Dict:
        """Turn the device on."""
        return self.set_relay_child_state(alias, child, 1)

    def turn_off_child(self, alias: str, child: str) -> Dict:
        """Turn the device off."""
        return self.set_relay_child_state(alias, child,0)
...
def main():
    ...
    parser.add_argument(
        ...
        choices=[
            'list', 'turnon', 'turnoff', 'powercycle', 'sysinfo',
            'turnonchild', 'turnoffchild', 'ison', 'isoff'
        ],
    ...
    if args.command == 'list':
        devices = None
        try:
            print("tplinktplink",tplink.get_device_list())
            devices = tplink.get_device_list()
    ...
    try:
        if args.command == 'turnoff':
            ...
            pp.pprint(result)
        if args.command == 'turnoffchild':
            result = tplink.turn_off_child(args.device , args.child)
            pp.pprint(result)
        elif args.command == 'turnonchild':
            # print("DOMNEDOMNEDOMNE")
            result = tplink.turn_on_child(args.device, args.child)
            pp.pprint(result)
